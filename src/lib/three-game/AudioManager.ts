...
export class AudioManager {
  ...
  loadSound(name: string, path: string, loop: boolean = false, volume: number = 1.0): void {
    ...
    if (this.soundLoadErrors.has(name)) {
      console.warn(`No se intentará cargar el sonido ${name} nuevamente debido a un error previo`);
      return;
    }

    // Si el sonido no está en SOUND_PATHS, no lo cargamos
    if (!(name in SOUND_PATHS)) {
      console.warn(`No se cargará el sonido ${name} porque no está definido en SOUND_PATHS`);
      return;
    }
  ...
  playSound(name: string): void {
    ...
    if (!(name in SOUND_PATHS)) {
      console.warn(`No se reproducirá el sonido ${name} porque no está definido en SOUND_PATHS`);
      return;
    }
    ...
    if (sound) {
      ...
      console.warn(`AudioManager: Sonido '${name}' no encontrado. Sonidos disponibles:`, 
  ...
  stopSound(name: string): void {
    ...
    if (sound) {
      ...
      console.warn(`AudioManager: No se pudo detener el sonido '${name}' - no encontrado`);
    }
  }
