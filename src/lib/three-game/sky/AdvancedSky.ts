...
export class AdvancedSky {
  ...
  constructor(
    ...
    const finalSunOrbitalRadius =
      skyElementsBaseRadius * (options.sunOrbitalRadiusFactor ?? 0.8);
    const finalMoonOrbitalRadius =
      skyElementsBaseRadius * (options.moonOrbitalRadiusFactor ?? 0.75);
    const finalSunSize =
      skyElementsBaseRadius * 0.05 * (options.sunSizeFactor ?? 1.0);
    const finalMoonSize =
    ...
    this.celestialBodyController = new CelestialBodyController(
      this.timeManager
    );
