...
export default function ErrorBoundaryDisplay({ title, message, onClose }: ErrorBoundaryDisplayProps) {
  return (
    <AlertDialog open={true} onOpenChange={(open) => !open && onClose()}>
      <AlertDialogContent className="max-w-2xl bg-background/95 backdrop-blur-sm">
        ...
        <AlertDialogDescription className="whitespace-pre-wrap text-sm max-h-[60vh] overflow-y-auto">
          <p className="mb-2 text-foreground">Ocurrió un error en el juego. Por favor, copia el siguiente mensaje para ayudar a depurarlo:</p>
          <textarea
            readOnly
