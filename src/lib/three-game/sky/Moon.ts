...
export class Moon implements ICelestialBody {
  ...
  update(timeNormalized: number, cameraPosition: THREE.Vector3): void {
    ...
    let isNightVisiblePhase1 =
      timeNormalized >= nightPortionStart && timeNormalized <= 1.0; // Evening to midnight
    let isNightVisiblePhase2 =
      timeNormalized >= 0.0 && timeNormalized <= nightPortionEnd; // Midnight to morning
    ...
    if (this.renderData.isVisible) {
      ...
      if (timeNormalized >= nightPortionStart) {
        ...
        moonProgress =
          (timeNormalized - nightPortionStart) /
          (1.0 - nightPortionStart + nightPortionEnd);
      } else {
        // current time is from 0.0 to nightPortionEnd (e.g. 0.25)
        // moonProgress continues from 0.5 (at 0.0) to 1.0 (at 0.25)
        moonProgress =
          (1.0 - nightPortionStart + timeNormalized) /
          (1.0 - nightPortionStart + nightPortionEnd);
      }
      ...
      this.renderData.position.x =
        Math.cos(moonAngle) * this.orbitalPathRadius * 0.9; // East-West movement
      this.renderData.position.y =
        Math.sin(moonAngle) * this.orbitalPathRadius * 0.5; // Up-Down movement
      this.renderData.position.z =
        -Math.sin(moonAngle) *
        Math.cos(moonAngle) *
        this.orbitalPathRadius *
        0.3; // Slight North-South wobble
      ...
      this.renderData.position.y = Math.max(
        cameraPosition.y - this.orbitalPathRadius * 0.15,
        this.renderData.position.y
      );
      ...
      this.renderData.intensity =
        horizonIntensity +
        (peakIntensity - horizonIntensity) * moonHeightFactor;
    } else {
