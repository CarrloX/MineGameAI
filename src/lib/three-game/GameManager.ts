...
export class GameManager {
  ...
  public update(newFpsValue?: number): void {
    ...
    refs.player.updatePosition(); // Updates player's internal x,y,z and camera position
    // InputHandler calls player.lookAround() which updates player's pitch/yaw and camera rotation
    ...
    if (refs.player.dead) {
      ...
      if (refs.camera && refs.player) {
        ...
        refs.player.lookAround(); // Apply restored pitch/yaw to camera
      }
    }
