...
export class PlayerMovementService {
    ...
    public updatePosition(deltaTime: number): void {
        ...
        let { nextPlayerX, nextPlayerZ } = this.calculateNextPosition(moveX, moveZ, currentEffectiveSpeed, deltaTime);
        ...
        this.applyFinalPosition(correctedX, correctedY, correctedZ, landedOnGroundThisFrame);
    }
    ...
    private calculateVerticalMovement(deltaTime: number): number {
        ...
        if (this.player.flying) {
            ...
            if (this.player.isFlyingAscending) dY += CONTROL_CONFIG.FLY_SPEED * deltaTime;
            if (this.player.isFlyingDescending) dY -= CONTROL_CONFIG.FLY_SPEED * deltaTime;
        } else {
            if (this.player.jumping && this.player.onGround) {
                this.player.jumpVelocity = CONTROL_CONFIG.JUMP_SPEED;
                this.player.onGround = false;
                this.player.jumping = false;
                if (this.player.audioManager) this.player.audioManager.playSound('jump');
            }
    ...
    private calculateNextPosition(moveX: number, moveZ: number, currentEffectiveSpeed: number, deltaTime: number): 
    ...
    private handleCollisions(nextPlayerX: number, nextPlayerY: number, nextPlayerZ: number): 
        { correctedX: number, correctedY: number, correctedZ: number, landedOnGroundThisFrame: boolean } {
        ...
        for (let checkWorldX = Math.floor(pMinProposedGlobalX) - checkRadius; 
             checkWorldX <= Math.ceil(pMaxProposedGlobalX) + checkRadius; 
             checkWorldX++) {
            for (let checkWorldZ = Math.floor(pMinProposedGlobalZ) - checkRadius; 
                ...
                for (let checkWorldY = startBlockY; checkWorldY < endBlockY; checkWorldY++) {
                    ...
                    if (blockType && blockType !== 'air' && blockType !== 'waterBlock') {
                        const collisionResult = this.resolveCollision(
                            checkWorldX, checkWorldY, checkWorldZ,
    ...
    private resolveCollision(
        ...
        blockType: string
    ): { correctedX: number, correctedY: number, correctedZ: number, landedOnGround: boolean } {
        const bMinX = blockX;
        ...
        if (pMaxX > bMinX && pMinX < bMaxX &&
            ...
            if (overlapY <= overlapX && overlapY <= overlapZ) {
                if (this.player.flying) {
                    ...
                    } else if (this.player.jumpVelocity === 0 && pMaxY > bMinY && pMinY < bMaxY) {
                        correctedY = (this.player.y > bMinY + this.player.height / 2) ? 
                            (bMaxY + 0.001) : (bMinY - this.player.height - 0.001);
                    }
                    this.player.jumpVelocity = 0;
                } else {
                    if (this.player.jumpVelocity <= 0 && pMinY < bMaxY - 0.0001 && 
    ...
    private applyFinalPosition(correctedX: number, correctedY: number, correctedZ: number, 
                             landedOnGroundThisFrame: boolean): void {
