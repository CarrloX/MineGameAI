...
export class AudioConfig {
    ...
    public getVolume(type: 'master' | 'music' | 'sfx' | 'ambient'): number {
        const audioConfig = this.config.get('audio') as AudioConfigData;
        switch (type) {
            case 'master': return audioConfig?.masterVolume ?? 1.0;
            case 'music': return audioConfig?.musicVolume ?? 0.7;
            case 'sfx': return audioConfig?.sfxVolume ?? 0.8;
            case 'ambient': return audioConfig?.ambientVolume ?? 0.5;
            default: return 1.0;
        }
    }

    public setVolume(type: 'master' | 'music' | 'sfx' | 'ambient', value: number): void {
        const audioConfig = this.config.get('audio') as AudioConfigData;
        if (audioConfig) {
            switch (type) {
                case 'master': audioConfig.masterVolume = value; break;
                case 'music': audioConfig.musicVolume = value; break;
                case 'sfx': audioConfig.sfxVolume = value; break;
                case 'ambient': audioConfig.ambientVolume = value; break;
            }
