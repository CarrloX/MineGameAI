...
export class AudioService implements IAudioService {
    ...
    private loadAllSounds(): void {
        ...
        Object.entries(soundPaths).forEach(([name, path]) => {
            if (path) { // Solo cargar sonidos que tengan una ruta definida
    ...
    public loadSound(name: string, path: string, loop: boolean = false, volume: number = 1.0): void {
    ...
    public playSound(name: string): void {
        ...
        if (sound) {
            ...
            console.warn(`AudioService: Sonido '${name}' no encontrado. Sonidos disponibles:`, 
    ...
    public stopSound(name: string): void {
        ...
        if (sound) {
            ...
            console.warn(`AudioService: No se pudo detener el sonido '${name}' - no encontrado`);
        }
    }
