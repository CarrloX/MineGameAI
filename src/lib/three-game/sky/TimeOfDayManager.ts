...
export class TimeOfDayManager implements ITimeProvider {
  ...
  constructor(cycleDurationMinutes: number = 20, initialTimeNormalized: number = 0.25) { // Default to 20 min, start at 6 AM
  ...
  public setTimeNormalized(normalizedTime: number): void {
    this.currentTime = Math.max(0, Math.min(1, normalizedTime)) * this.cycleDurationSeconds;
  }
}
